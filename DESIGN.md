...
## Public Positioning & Audience

Deliberately **not** a public multi-tenant service. Concretely:

- **No live, publicly-hosted upload endpoint.** Two real reasons, not just caution: (1) cost — every AI review is a paid Claude API call, and an open endpoint means paying for every stranger who finds the link; (2) security — the parser was built to be correct against well-formed KiCad files, not hardened against adversarial input from the open internet, and accepting arbitrary uploads from strangers is a real attack surface that hasn't been addressed.
- **The actual use case is personal.** The honest interview narrative: "I design PCBs as a hobby, and built my own AI-augmented review tool to check them before sending anything to fab — same design-review step a PCB team runs before tapeout, except I built the reviewer myself." That's a genuine, defensible story, not a cover story for a demo.
- **GitHub repo stays the primary public artifact** — code, tests, docs, the example report — browsable by anyone with the link (recruiters, interviewers). Not advertised for public signups or general use. Interested parties can email for a live demo/access rather than self-serve.
- **Phase 4's public-facing deliverable is a static landing page**, not a live hosted instance: screenshots, the architecture diagram, and a clickable pre-generated example report. This gets the visual portfolio benefit of a dashboard without the hosting-cost/security commitment of actually running one publicly. The functional React/FastAPI dashboard (see Phase 4 below) still gets built for real, local, personal use — it's the landing page that stays static, not the tool itself. Build order: the real dashboard first, so the landing page's screenshots are genuine product photos, not mockups. Deliberately static rather than a fully 3D/interactive experience — matches the dashboard's own already-chosen GitHub/Linear/JetBrains-style aesthetic (no glassmorphism, no gradients), loads fast, and doesn't risk reading as style-over-substance for a project whose whole pitch is engineering rigor. A recruiter's evaluation window is short; friction (load time, GPU dependency) works against the project, not for it.
- **PCB design workflow is documented, not AI-generated.** The README/docs note which tool the boards are actually designed in (KiCad) and which fab house they'd be ordered from (e.g. JLCPCB/PCBWay) — supporting the "I design PCBs as a hobby and built my own reviewer" narrative honestly. Deliberately did not add an AI-PCB-design feature: this project's entire credibility story rests on "the AI reviews, it never designs" (see Constraints and "AI Integration Architecture" above), and having Claude generate new PCB designs would directly contradict that positioning, not extend it.
...
**Phase 4 — React/TypeScript dashboard. (DONE)** `backend/app/main.py` exposes `POST /api/review` (multipart upload with streamed/size-capped writes and a `Path.name` traversal guard -> parse -> `run_all_checks` -> score -> optional AI review -> JSON), `POST /api/chat` (reuses `answer_question()`), and `POST /api/report/pdf` (ReportLab, reusing the HTML report's charts), all local-only per the Public Positioning decision above (CORS scoped to the local Vite dev server). `frontend/src/` is a working dashboard: drag-and-drop/folder-picker upload, color-coded overall/subscore cards, a searchable/filterable issue browser with expandable `explanation`/`principle`/`suggested_fix`/confidence cards (the Rule Explanation UI stretch goal, free since `Issue` already carried that data), an SVG board visualization (outline, per-layer traces, vias, components, zoom/pan, layer toggles, and clickable issue-location markers that cross-highlight the issue browser), a net-length histogram and issue-by-category chart, a grounded AI chat panel (fails gracefully without an API key), and a "Download PDF" button. Verified end-to-end in a real browser against the `examples/simple_board` fixture through the actual running backend, not mocked.
